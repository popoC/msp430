...
#define TimeOut 1500
  ...
  unsigned char RTC_Serial[7]; // int ms,sec,minute,hour,day,month,year;
...
void main(void)
{
  ...
  while(!StartRecord){
         ...
         switch(OUTPUT_MODE){
           case LOGGER_TIME:
                     string[0] = 't';                  //資料型態
                     string[8] = 0; string[9] = 0;  string[10] = 1;  
              get_time();      memcpy(&(string[1]),RTC_Serial,7);
...
int scLk,ch_n,delAy;
  ...
  //-----=-----時間請由這裡修正 第24個SCLK HI 先不轉態~ 最大不超過 1/120 s 這時段可以先抓時間後再轉態
  /* if(save_address == 0)gettime();   saveaddress = 0 
...
void get_time(){
   ...
   P6OUT &= ~BIT3;
 for(int i=1;i<7;i++) RTC_Serial[i] = get_byte();
