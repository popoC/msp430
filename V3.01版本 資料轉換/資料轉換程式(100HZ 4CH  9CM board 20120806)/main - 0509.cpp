...
//--------function-----------------------------------------------------------
unsigned __int64 Find_TimeSN_Compress_v2(String Tstr);
...
int Find_SD_Sector_Data_Compress(int32_t *Raw,unsigned __int64 SectorNum,int ch);
...
double timediff;
       ...
       t1 =    Read_OBS_SD_Sector_Time(s1+0);
       t2 =    Read_OBS_SD_Sector_Time(s1+1);
       t3 =    Read_OBS_SD_Sector_Time(s1+2);

                                 //--每個sector(512Byte)間格時間3Ch/120Hz 不可能超過 1S = (hptime)1000000~
 ...
 data_no1  = Find_SD_Sector_Data_Compress( RawData  ,s1,ch);
...
// Form1->Memo2->Lines->Add(data_no1);
 ...
 data_no2 = Find_SD_Sector_Data_Compress( RawData ,s1+1,ch);
...
int Find_SD_Sector_Data_Compress(int32_t *Raw,unsigned __int64 SectorNum,int ch){
  ...
  if( hDevice == INVALID_HANDLE_VALUE ){ ShowMessage("Device Handle Error!!");
   ...
   unsigned char SdBuff[512];
    ...
    lPpoint.LowPart  = (SectorNum & 0x00000000FFFFFFFF);  lPpoint.HighPart =((SectorNum & 0xFFFFFFFF00000000)>>32);
  ...
  int data_in_buff = SdBuff[7];    //--- 此處修正 因為MSP那端已寫好 SdBuff[7]內的值 為資料數量
...
unsigned __int64 Find_TimeSN_Compress(String Tstr){
  ...
  if( (t2 > t3) || (t2 < t1) )       { ShowMessage("不在範圍內");  return(0); }
   ...
   t2_b = Read_OBS_SD_Sector_Time(Search_Sector);
   ...
   T_err = abs( t2 -  t2_b);
    ...
    //  Form1->Memo2->Lines->Add(errcounter);
       if(t2_b < t2){
         return(Search_Sector+1);}
...
int32_t RawData[Pack_N];
   ...
   unsigned __int64 s1,s2,s_buff,cut_time;
           ...
           end_t = Read_OBS_SD_Sector_Time(s2);
...
int Out_Miniseed(String filename,unsigned __int64 m_start_sn,unsigned __int64 m_end_sn){
   ...
   unsigned __int64 s_buff,cut_time;
           ...
           end_t   = Read_OBS_SD_Sector_Time(m_end_sn);
           start_t = Read_OBS_SD_Sector_Time(m_start_sn);
...
int filecount = 0;         
 ...
 Start_n = Find_TimeSN_Compress( Edit8->Text);    //    找出對應時間的sector序號
 End_n = Find_TimeSN_Compress( Edit9->Text);      //    Memo1->Lines->Add(Out_Miniseed("t67_311.mseed",Sn,En));


    start_t = Read_OBS_SD_Sector_Time(Start_n);      //    須轉換為整數型態方便運算
    end_t   = Read_OBS_SD_Sector_Time(End_n);        //    回傳值為該Sector所記錄之時間(格式為hptime_t)
 ...
 //cut_time = end_t-start_t+1000000+Add_overlay_time;
        ...
        int cut_mins = (Edit10->Text.ToInt()%60);
         ...
         strT_string = "";
           ...
           B_n = Find_TimeSN_Compress( str_buf);
...
void __fastcall TForm1::Button2Click(TObject *Sender)
{
 ...
 Out_Miniseed_v2("Thehsa.**.mseed",23256125,23267549);
...
{
           Memo1->Lines->Add(Find_TimeSN_Compress( Edit1->Text));
           Memo2->Lines->Add(Find_TimeSN_Compress( Edit2->Text));
}
//---------------------------------------------------------------------------
...
char str_buf[30];
 ms_hptime2mdtimestr (Read_OBS_SD_Sector_Time( Edit14->Text.ToDouble() ), str_buf, 1) ;

    Memo1->Lines->Add(String(str_buf));
}
//---------------------------------------------------------------------------
...
int HJ;
HJ =Edit13->Text.ToDouble() - Edit12->Text.ToDouble();
s1 =  Edit12->Text.ToDouble();
 

  for(int i=0;i<HJ;i++){


       t1 =    Read_OBS_SD_Sector_Time(s1+i);

       t2 =    Read_OBS_SD_Sector_Time(s1+i+1);

       if((t2-t1)>1000000){

       Memo1->Lines->Add(s1+i);
       Memo2->Lines->Add(t2-t1);
       }
...
for(int i=0;i<512;i++){
 ...
 Raw_Data[i] = new Obs_Sd_Data[3];

}
 Raw_Data[0][0].data_no =  0;

unsigned __int64 s1;
  s1 =  Edit12->Text.ToDouble();

int datano =    Get_SD_Sector_Data_DeCompress(Raw_Data,s1);



  for(int i=0;i<datano;i++){

   Series1->Add(Raw_Data[i][2].data,i,clRed);

  }

  Memo1->Lines->Add( Raw_Data[0][0].data_no) ;
...
{
           Memo1->Lines->Add(Find_TimeSN_Compress_v2( Edit1->Text));
           Memo2->Lines->Add(Find_TimeSN_Compress_v2( Edit2->Text));        
}
//---------------------------------------------------------------------------
...
Series1->Clear();
Obs_Sd_Data **Raw_Data = new Obs_Sd_Data*[512];
   for(int i=0;i<512;i++) Raw_Data[i] = new Obs_Sd_Data[3];

Raw_Data[0][0].data_no =  0;

unsigned __int64 s1,s2;
  s1 =  Find_TimeSN_Compress_v2( Edit1->Text);
  s2 =  Find_TimeSN_Compress_v2( Edit2->Text);
...
int datano =    Get_SD_Sector_Data_DeCompress(Raw_Data,s1);
  ...
  for(int i=0;i<datano;i++){
   ...
   Series1->AddXY(Raw_Data[i][2].time,Raw_Data[i][2].data,clRed);
  ...
  hptime_t New_find_t = ms_timestr2hptime(Edit1->Text.c_str());     // hptime      2012_0501 /
   ...
   int32_t Find_Data[3];
       while( New_find_t <  end_t){
        ...
        Line_Interpolation(New_find_t,&s1,Raw_Data,Find_Data);


        Series2->AddXY(New_find_t,Find_Data[2],clGreen);
...
//---------------------------------------------------------------------------
unsigned __int64 Out_Miniseed_v3(String filename,unsigned __int64 search_sn,hptime_t start_t,hptime_t end_t)
{             // filename 輸出檔案名稱
   ...
   int32_t RawData[Pack_N];
        ...
        int packedsamples;
         ...
         outfile = fopen(StringReplace(filename,"**","HHZ",TReplaceFlags()).c_str(),"wb");
            ...
            int datano =    Get_SD_Sector_Data_DeCompress(Raw_Data,search_sn);
            int32_t Find_Data[3];
         ...
         if(pk_N >=Pack_N)
            {
               ...
               msr_pack(msr, &record_handler, NULL, &packedsamples, 1, 0);
...
unsigned __int64 s1,s2;
  ...
  hptime_t New_find_t = ms_timestr2hptime(Edit1->Text.c_str());     // hptime      2012_0501 /
      ...
      Out_Miniseed_v3("Thehsa.**.mseed",s1,New_find_t,end_t);
}
//---------------------------------------------------------------------------
