...
//------------------------------------------------------------------
  ...
  void TDCM3_Enable(){
      ...
      if(TDCM3_Calibration){
        ...
        P2OUT &= ~TDCM3_RX;
        P2OUT &= ~TDCM3_RTS;   //--開始
          for(delay1=0;delay1<1000;delay1++);   //---- 800條指令約 1ms

        P2OUT |= TDCM3_RTS;   //--

        tdcm3_N = 0;
        
        for(delay1=0;delay1<50;delay1++)
        for(delay2=0;delay2<800;delay2++);

        if((TDCM3_Buff[0]==0x41)&&(TDCM3_Buff[1]==0x43)&&(TDCM3_Buff[2]==0x4B)){
          for(delay1=0;delay1<250;delay1++)
          for(delay2=0;delay2<800;delay2++);        //---此處.250ms的時間..旋轉吧~~~~
        }
        else{
       ...
       if((TDCM3_Buff[0]==0x52)&&(TDCM3_Buff[1]==0x44)&&(TDCM3_Buff[2]==0x59)){

         TDCM3_Calibration = false;         
       }
       else{
         TDCM3_Calibration = true;
       }
      ...
      if(TDCM3_Buff[0]==0x81){
  
        TDCM3_status = TDCM3_Buff[0];     
        TDCM3_Heading =  (TDCM3_Buff[1]*256+TDCM3_Buff[2])/2;
        
        TDCM3_Calibration = true;   //--需要校正
      }
