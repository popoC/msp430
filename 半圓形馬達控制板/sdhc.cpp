...
//        sd_buffer = (unsigned char *)malloc(2048*sizeof(unsigned char));
...
int sd_read_cid (sd_context_t *sdc, unsigned char *data)
{
	unsigned char blank = 0xFF;
	...
	/* Need to add size checking */
	if (sd_send_command(sdc, CMD10, CMD10_R, response, argument) == 0)
		return 0;
...
int sd_read_csd (sd_context_t *sdc, unsigned char *data)
{
	unsigned char blank = 0xFF;
	...
	/* Need to add size checking */
	if (sd_send_command(sdc, CMD9, CMD9_R, response, argument) == 0)
		return 0;
