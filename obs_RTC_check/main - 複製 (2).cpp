   ...
   //  __bis_SR_register(GIE+LPM0_bits);
     ...
     if(OBS_Time_Flag){
          ...
          OBS_Time_Flag = 0;
           ...
           //  nEw_ms =   ((float)InBuffer[7] / 125)*1000 + ((float)InBuffer[8]/10);
           OBS_ms =  ((float)COM3_BUFFER[7] / 125)*1000 + ((float)COM3_BUFFER[8]/10); 
           OBS_Time_hp = OBS_Time_hp + (hptime_t)(OBS_ms*1000); 
           
           
         diff_Time_hp  =   (sysTime+OBS_10k_Buffer*100) - OBS_Time_hp;
          ms_hptime2mdtimestr(diff_Time_hp, diff_Time_String, 1);
         //   ms_hptime2mdtimestr(sysTime, diff_Time_String, 1);
          
          diff_Time_String[26] = '\r';
          diff_Time_String[27] = '\n';
    ...
    /*      */
     ...
     if(sysTime_flag == 1){
                   sysTime_Buffer = get_hp_Gps_time(COM2_BUFFER);
                   sysTime_flag = 2;
...
__interrupt void USCI_A2_ISR(void)
{
  ...
  switch(__even_in_range(UCA2IV,4))
  {
     case 2:                                   // Vector 2 - RXIFG
       ...
       else if(COM3_REC_BUFFER[UART_COM3_RX_count-1] == 1){
            UART_COM3_RX_count = 0;
       }
